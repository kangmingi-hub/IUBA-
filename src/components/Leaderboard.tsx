...
const ClubCard = memo(({ club, player, ownedCount, totalBuildings }: {
  club: { club_name: string; remaining_evangelism_points: number; remaining_speech_points: number; };
  player: Player | undefined;
  ownedCount: number;
  totalBuildings: number;
}) => (
  <div className={cn(
    "flex flex-col gap-2.5 p-3.5 rounded-2xl border",
    ownedCount > 0 ? "border-blue-300/40 bg-white/85" : "border-white/30 bg-white/80"
  )}>
...
export default function Leaderboard({ clubPoints, players, countries, isSyncing, onRefresh, onReset, onResetManual, currentUser }: Props) {
  ...
  const DURATION_PER_ITEM = 3;
  const totalDuration = filteredClubs.length * DURATION_PER_ITEM;

  const getCardProps = (club: typeof filteredClubs[0]) => {
    ...
    const totalBuildings = player
      ...
      : 0;
    return { player, ownedCount, totalBuildings };
  };
  ...
  return (
    ...
    >
      ...
      {/* 뷰포트 — overflow hidden으로 복제본 안 보이게, 터치하면 멈춤 */}
      <div
        ...
        {filteredClubs.length > 0 && (
          <div className={cn("marquee-track space-y-2.5", paused && "paused")}>
            {filteredClubs.map((club) => {
              const { player, ownedCount, totalBuildings } = getCardProps(club);
              return <ClubCard key={`a-${club.club_name}`} club={club} player={player} ownedCount={ownedCount} totalBuildings={totalBuildings} />;
            })}
            {filteredClubs.map((club) => {
              const { player, ownedCount, totalBuildings } = getCardProps(club);
              return <ClubCard key={`b-${club.club_name}`} club={club} player={player} ownedCount={ownedCount} totalBuildings={totalBuildings} />;
            })}
          </div>
        )}
      </div>
