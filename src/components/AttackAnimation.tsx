...
export default function AttackAnimation({ players }: Props) {
  // visibleEvents: 실제로 화면에 표시되는 이벤트 (30초 후에 추가됨)
  const [visibleEvents, setVisibleEvents] = useState<AttackEvent[]>([]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (shownIds.has(newEvent.id)) return;
        ...
        setTimeout(async () => {
          ...
          if (newEvent.result === 'conquered') {
            await supabase.from('country_occupations')
              ...
              .eq('country_id', (newEvent as any).country_id);
          }
          ...
          // 카드 표시 후 5초 뒤에 제거
          setTimeout(() => {
            setVisibleEvents(prev => prev.filter(e => e.id !== newEvent.id));
          }, 5000);
